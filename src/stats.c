...
void printGlobalStats(struct config* config) {
  ...
  //} 
  //printf("\n");
  printf("%10.1f, %9.1f, %10d, %10d, %10.3f, %10.3f, %10.3f, %10.3f\n", 
		timeDiff, rps, global_stats.gets, global_stats.sets,
		1000*getAvg(&global_stats.response_time), 1000*q99, 1000*global_stats.response_time.min, 1000*global_stats.response_time.max);
...
void statsLoop(struct config* config) {
  ...
  printf("Stats:\n");
  printf("-------------------------\n");
  printf("%10s,%8s,%8s,%11s,%10s,%12s,%10s,%11s\n", "timeDiff", "rps", "gets", "sets",  "avg_lat", "99th", "min", "max");
  while(1) {
